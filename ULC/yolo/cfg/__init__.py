...
from ULC.yolo.utils import (DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, ROOT, USER_CONFIG_DIR,
                                    IterableSimpleNamespace, __version__, checks, colorstr, yaml_load, yaml_print)

CLI_HELP_MSG = \
    f"""
    Arguments received: {str(['yolo'] + sys.argv[1:])}. ULC 'yolo' commands use the following syntax:
...
def entrypoint(debug=''):
    """
    This function is the ULC package entrypoint, it's responsible for parsing the command line arguments passed
    to the package.
