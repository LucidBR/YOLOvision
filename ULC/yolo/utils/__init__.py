...
class SimpleClass:
    """
    ULC SimpleClass is a base class providing helpful string representation, error reporting, and attribute
...
class IterableSimpleNamespace(SimpleNamespace):
    """
    ULC IterableSimpleNamespace is an extension class of SimpleNamespace that adds iterable functionality and
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date ULC
            'default.yaml' file.\nPlease update your code with 'pip install -U ULC' and if necessary replace
...
def get_user_config_dir(sub_dir='ULC'):
    """
...
USER_CONFIG_DIR = os.getenv('YOLO_CONFIG_DIR', get_user_config_dir())  # ULC settings dir
...
def get_settings(file=USER_CONFIG_DIR / 'settings.yaml', version='0.0.2'):
    """
    Loads a global ULC settings YAML file or creates one with default values if it does not exist.

    Args:
        file (Path): Path to the ULC settings YAML file. Defaults to 'settings.yaml' in the USER_CONFIG_DIR.
    ...
    from ULC.yolo.utils.checks import check_version
    from ULC.yolo.utils.torch_utils import torch_distributed_zero_first
    ...
    defaults = {
        ...
        'settings_version': version}  # ULC settings version
    ...
    with torch_distributed_zero_first(RANK):
        ...
        if not correct:
            LOGGER.warning('WARNING ⚠️ ULC settings reset to defaults. This is normal and may be due to a '
                           'recent ULC package update, but may have overwritten previous settings. '
...
def set_settings(kwargs, file=USER_CONFIG_DIR / 'settings.yaml'):
    """
    Function that runs on a first-time ULC package installation to set up global settings and create necessary
    directories.
    """
