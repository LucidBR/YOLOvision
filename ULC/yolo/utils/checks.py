...
from ULC.yolo.utils import (AUTOINSTALL, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, TryExcept, colorstr, downloads,
...
def check_latest_pypi_version(package_name='ULC'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ULC package is available on PyPI.
    ...
    if ONLINE and is_pip_package():
        with contextlib.suppress(ConnectionError):
            ...
            if pkg.parse_version(__version__) < pkg.parse_version(latest):  # update is available
                LOGGER.info(f'New https://pypi.org/project/ULC/{latest} available 😃 '
                            f"Update with 'pip install -U ULC'")
                return True
    return False
...
def check_yolo(verbose=True, device=''):
    from ULC.yolo.utils.torch_utils import select_device
