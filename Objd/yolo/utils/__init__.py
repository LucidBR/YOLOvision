...
class SimpleClass:
    """
    Objd SimpleClass is a base class providing helpful string representation, error reporting, and attribute
...
class IterableSimpleNamespace(SimpleNamespace):
    """
    Objd IterableSimpleNamespace is an extension class of SimpleNamespace that adds iterable functionality and
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date Objd
            'default.yaml' file.\nPlease update your code with 'pip install -U Objd' and if necessary replace
...
def get_user_config_dir(sub_dir='Objd'):
    """
...
USER_CONFIG_DIR = os.getenv('YOLO_CONFIG_DIR', get_user_config_dir())  # Objd settings dir
...
def get_settings(file=USER_CONFIG_DIR / 'settings.yaml', version='0.0.2'):
    """
    Loads a global Objd settings YAML file or creates one with default values if it does not exist.

    Args:
        file (Path): Path to the Objd settings YAML file. Defaults to 'settings.yaml' in the USER_CONFIG_DIR.
    ...
    from Objd.yolo.utils.checks import check_version
    from Objd.yolo.utils.torch_utils import torch_distributed_zero_first
    ...
    defaults = {
        ...
        'settings_version': version}  # Objd settings version
    ...
    with torch_distributed_zero_first(RANK):
        ...
        if not correct:
            LOGGER.warning('WARNING ⚠️ Objd settings reset to defaults. This is normal and may be due to a '
                           'recent Objd package update, but may have overwritten previous settings. '
...
def set_settings(kwargs, file=USER_CONFIG_DIR / 'settings.yaml'):
    """
    Function that runs on a first-time Objd package installation to set up global settings and create necessary
    directories.
    """
