...
from Objd.yolo.utils import (AUTOINSTALL, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, TryExcept, colorstr, downloads,
...
def check_latest_pypi_version(package_name='Objd'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the Objd package is available on PyPI.
        ...
        with contextlib.suppress(ConnectionError):
            ...
            if pkg.parse_version(__version__) < pkg.parse_version(latest):  # update is available
                LOGGER.info(f'New https://pypi.org/project/ULC/{latest} available 😃 '
                            f"Update with 'pip install -U Objd'")
                return True
    return False
...
def check_yolo(verbose=True, device=''):
    from Objd.yolo.utils.torch_utils import select_device
