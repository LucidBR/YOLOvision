...
from YOLOvision.yolo.vision.detection.train import Loss


# BaseTrainer python usage
class SegmentationTrainer(vision.detection.DetectionTrainer):

    def __init__(self, cfg=DEFAULT_CFG, overrides=None, *args, **kwargs):
        if overrides is None:
            overrides = {}
        overrides['task'] = 'segmentation'
    ...
    def get_validator(self, *args, **kwargs):
        ...
        return vision.segmentation.SegmentationValidator(self.test_loader, save_dir=self.save_dir, args=copy(self.args))
    ...
    def plot_metrics(self, *args, **kwargs):
        plot_results(file=self.csv, segmentation=True)  # save results.png


# Criterion class for computing training losses
class SegLoss(Loss):

    def __init__(self, model, overlap=True, *args, **kwargs):  # model must be de-paralleled
    ...
    def __call__(self, preds, batch, *args, **kwargs):
        ...
        try:
            ...
            raise TypeError('ERROR ❌ segmentation dataset incorrectly formatted or not a segmentation dataset.\n'
                            ...
                            "i.e. 'yolo train model=YOLOvisionn-seg.pt data=coco128.yaml'.\nVerify your dataset is a "
                            "correctly formatted 'segmentation' dataset using 'data=coco128-seg.yaml' "
                            'as an example.\nSee https://docs.ULC.com/tasks/segmentation/ for help.') from e
    ...
    def single_mask_loss(self, gt_mask, pred, proto, xyxy, area, *args, **kwargs):
