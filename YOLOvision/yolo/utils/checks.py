...
from YOLOvision.yolo.utils import (AUTOINSTALL, LOGGER, ONLINE, ROOT, USER_CONFIG_DIR, TryExcept, colorstr, downloads,
...
def check_latest_pypi_version(package_name='YOLOvision'):
    """
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    if s and install and AUTOINSTALL:  # check environment variable
        LOGGER.info(f"{prefix} YOLOvision requirement{'s' * (n > 1)} {s}not found, attempting AutoUpdate...")
        try:
...
def check_suffix(file='YOLOvisionn.pt', suffix='.pt', msg=''):
